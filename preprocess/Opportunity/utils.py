...
def sliding_window(a,ws,ss = None,flatten = True):
	...
	if None is ss:
		...
		ss = ws
	ws = norm_shape(ws)  # 这里的ws是窗口大小(30,113)
	ss = norm_shape(ss)
	...
	ws = np.array(ws)
	ss = np.array(ss)
	shape = np.array(a.shape)  # shape是整个文件的
	...
	if np.any(ws > shape):
		raise ValueError('ws cannot be larger than a in any dimension.a.shape was %s and ws was %s' % (str(a.shape),
																									   str(ws)))

	# how many slices will there be in each dimension?
	newshape = norm_shape(((shape - ws) // ss) + 1)  # type(newshape) tuple
...
#     dim = filter(lambda i : i != 1,dim)
	return strided.reshape(dim)  # 如果输出的是y, 那每一行y都是一个相同的数字
